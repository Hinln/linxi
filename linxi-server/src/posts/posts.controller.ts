...
export class PostsController {
  ...
  @Public()
  @Get('home')
  async getHomeFeed(@Query() paginationDto: PaginationDto, @Request() req) {
  ...
  @Public()
  @Get('nearby')
  async getNearby(@Query() nearbyDto: NearbyDto, @Request() req) {
    const userId = req.user?.userId;
    return this.postsService.getNearby(nearbyDto, userId);
  }

  @Post(':id/report')
  async report(@Request() req, @Param('id') id: string, @Body() body: { reason: string }) {

...
export class RealPersonService {
   ...
   * @returns CertifyId
   */
  async initFaceVerify(outerOrderNo: string, sceneId: string, metaInfo: string): Promise<string> {
    const initRequest = new $CloudAuth20190307.InitFaceVerifyRequest({
      sceneId: Number(sceneId),
      outerOrderNo: outerOrderNo,
      productCode: "ID_PRO", // Or from env
      metaInfo: metaInfo, // Pass metaInfo from client
    });
